...
from codes.models.encoder.clip.simple_tokenizer import SimpleTokenizer as _Tokenizer
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisualTransformer(
                input_resolution=image_resolution,
                patch_size=vision_patch_size, # 32
...
if __name__ == '__main__':
    ...
    # patch_size = 32, vision_width = 768, vision_layers = 12, vision_heads = 12, output_dim = 512, vocab_size = 49408
    model, preprocess = load_clip("/home/zzy/pycharm/projects/vlt_grasp/tac-grasp-net/weights/ViT-B-32.pt", device=device)

    image = preprocess(Image.open("/home/zzy/图片/CLIP.png")).unsqueeze(0).to(device)
    text = tokenize(["a diagram"]).to(device)
    print(text.shape)

    with torch.no_grad():
        image_features = model.encode_image(image)  # [N, 512]
        text_features = model.encode_text(text)     # [N, 512]
