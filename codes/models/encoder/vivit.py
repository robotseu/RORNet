...
class FDPAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv_s = qkv_s.reshape(B * T, HW, 3, self.num_heads // 2, (
                    D * 2) // self.num_heads)  # Batch_size*frames(time), height*width, 3, num_heads//2, D*2//self.num_heads
        ...
        qkv_t = qkv_t.reshape(B * HW, T, 3, self.num_heads // 2, (
                    D * 2) // self.num_heads)  # Batch_size* height*width, frames(time), 3, num_heads//2, D*2//self.num_heads
...
class FDTBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = FDPAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                                 proj_drop=drop)  # Factorized Dot-Product Attention
...
class UniformPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(
            x)  # Create patches using convolution. Trick from the timeSFormer paper. Batch, embed_dim, num_patch_across_height, num_patch_across_width
...
class VIVIT(nn.Module):
    def __init__(self, Tactile_img_size=(224, 224), Tactile_patch_size=(16, 16), in_chans=3, num_classes=3,
                 embed_dim=512, depth=3, num_heads=8, mlp_ratio=4, qkv_bias=False, qk_scale=None, drop_rate=0.1,
                 attn_drop_rate=0.1, drop_path_rate=0.1, num_frames=8, dropout=0.1, use_gpu=True):
        ...
        # x -> visual image
        # y -> tactile image
        self.patch_embed_y = UniformPatchEmbed(img_size=Tactile_img_size, patch_size=Tactile_patch_size,
